...
// Execution:
//  gcc -o playfair playfair.c
// 	.\playfair.exe


// Algorithm:  
// a. Key Matrix Generation:  
// -	Create a 5x5 matrix based on a given key.  
// -	Fill the matrix with the unique letters of the key (ignoring duplicates) and then fill the remaining cells with the remaining letters of the alphabet.  

// b. Text Preprocessing:  
// -	Convert the input text to uppercase.  
// -	Replace any 'J' with 'I'.  
// -	Break the text into pairs of two letters (diagram).  

// c. Encryption:  - For each diagram:  
// -	If the letters are in the same row, replace them with the letters to their immediate right (circular). 
// -    If the letters are in the same column, replace them with the letters immediately below (circular).  
// -	If the letters form a rectangle, replace them with the letters in the same row but at the column of the other letter and vice versa.  
// -	Append the modified diagram to the encrypted text.  

// d. Decryption:  
// - Perform the reverse operation of encryption to decrypt the text. 
